...
class ClassName:
    ...
    def _load_config(self):
        ...
        try:
            ...
            self.quantity = int(config['GooglePDFSearch']['StartQuantity'])
    ...
    def search(self):
        ...
        while self.counter <= self.limit and self.counter <= 100:
            time.sleep(1)
            if self.verbose:
                logging.info('Google PDF Search on page: %d', self.counter)

            urly = f"https://www.google.com/search?q={self.domain}+filetype:pdf&start={self.counter}"
            try:
                r = requests.get(urly)
                r.raise_for_status()
    ...
    def _download_files(self, dl, convert):
        for url in self.url_list:
            if self.verbose:
                logging.info('Google PDF search downloading: %s', url)
            try:
                filetype = ".pdf"
                file_name, file_download = dl.download_file(url, filetype)
                if file_download:
                    logging.info('Google PDF file downloaded: %s', url)
                    self.text += convert.convert_pdf_to_txt(file_name)
